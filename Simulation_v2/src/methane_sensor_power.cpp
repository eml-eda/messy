...
void methane_sensor_power::processing()
{
    if(func_signal.read() == 1 ){
        //std::cout << "methane_sensor in ON_WRITE state!" << std::endl;
        voltage_state.write(METHANE_SENSOR_VOLTAGE);
        current_state.write(METHANE_SENSOR_I_ON_WRITE);
        //power_to_therm.write(5.0);
        return;
    }

    if(func_signal.read() == 2 ){
        //std::cout << "methane_sensor in ON_READ state!" << std::endl;
        voltage_state.write(METHANE_SENSOR_VOLTAGE);
        current_state.write(METHANE_SENSOR_I_ON_READ);
        //power_to_therm.write(5.0);
        return;
    }

    if(func_signal.read() == 3 ){
        //std::cout << "methane_sensor in IDLE state!" << std::endl;
        voltage_state.write(METHANE_SENSOR_VOLTAGE);
        current_state.write(METHANE_SENSOR_I_IDLE);
        //power_to_therm.write(5.0);
        return;
    }

    if(func_signal.read() == 0){
        //std::cout << "methane_sensor in OFF state" << std::endl;
    ...
    std::cout << "methane_sensor in an Unknown state!" << std::endl;
    //power_to_therm.write(0.0);       
}